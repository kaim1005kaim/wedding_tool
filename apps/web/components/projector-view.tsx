...
const QuizBoard = memo(function QuizBoard({ activeQuiz, quizResult, leaderboard, phase, representatives, showRanking, mode }: QuizPanelProps) {
  ...
  const calculateRankingsWithTies = (entries: typeof quizLeaderboard | typeof buzzerRanking): any[] => {
    ...
    for (let i = 0; i < Math.min(entries.length, 12); i++) {
      ...
      if (i > 0) {
        ...
        const currentScore = 'latencyMs' in entry && entry.latencyMs != null ? entry.latencyMs : ('quizPoints' in entry ? entry.quizPoints : 0);
        const prevScore = 'latencyMs' in prevEntry && prevEntry.latencyMs != null ? prevEntry.latencyMs : ('quizPoints' in prevEntry ? prevEntry.quizPoints : 0);
  ...
  const rankedLeaderboard = calculateRankingsWithTies(isBuzzerQuiz ? buzzerRanking : quizLeaderboard);
  ...
  if ((phase === 'ended' || (showRanking && mode === 'quiz')) && hasRankingData) {
    ...
    const getRankStyle = (rank: number) => {
      switch (rank) {
        case 1: return {
          bg: 'bg-gradient-to-br from-yellow-50/90 to-orange-50/90',
          border: 'border-yellow-400',
          ring: 'ring-yellow-300/50',
          textColor: 'text-yellow-600'
        };
        case 2: return {
          bg: 'bg-gradient-to-br from-gray-50/80 to-slate-50/80',
    ...
    return (
      <motion.section
        ...
        exit={{ opacity: 0, y: -30 }}
        transition={{ duration: 0.5, ease: 'easeOut' }}
        className="flex h-full flex-col gap-6 px-8 py-6"
        role="region"
        aria-label="クイズランキング"
      >
        <motion.div
