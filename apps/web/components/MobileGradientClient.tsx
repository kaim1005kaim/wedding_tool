...
export default function MobileGradientClient({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const handleContextRestored = () => {
      console.log('WebGL context restored on mobile');
      setHasError(false);
    };
  ...
  if (hasError) {
    return (
      <div className={`fixed inset-0 -z-10 bg-gradient-mobile ${className}`} />
    );
  }
  ...
  return (
    <div className={`fixed inset-0 -z-10 ${className}`}>
      <Canvas
        ...
        onError={(error) => {
          console.error('Three.js mobile Canvas error:', error);
          setHasError(true);
        }}
      >
        <MobileGradientMesh />
